...
class FiniteFieldMath {
    constructor(primitivePoly = 0x11D) {
    ...
    gfMultiply(valA, valB) {
        if (valA === 0 || valB === 0) {
            return 0;
        }
        const logA = this.logarithmTable[valA];
        const logB = this.logarithmTable[valB];
        let sumLogs = logA + logB;
        if (sumLogs >= 255) {
            sumLogs -= 255;
        }
    ...
    polyMultiply(poly1, poly2) {
        const product = new Array(poly1.length + poly2.length - 1).fill(0);
    ...
    polyDivide(dividendPoly, divisorPoly) {
        let tempDividend = [...dividendPoly];
        const divisorLead = divisorPoly[0];
        const divisorLen = divisorPoly.length;
        
        for (let i = 0; i <= tempDividend.length - divisorLen; i++) {
            const factor = this.gfMultiply(tempDividend[i], this.gfInverse(divisorLead));
            for (let j = 0; j < divisorLen; j++) {
                tempDividend[i + j] ^= this.gfMultiply(divisorPoly[j], factor);
            }
        }

        const eccLength = divisorLen - 1;
        return tempDividend.slice(tempDividend.length - eccLength);
    }

    static generateRsPoly(gfInstance, numEccBytes) {
        let genPoly = [1];
...
export class CanvasLocal { 
    constructor(g, canvas) {
        ...
        this.allMaskableIndices = new Set();
    ...
    getModuleX(col) { return this.offsetX + col * this.modulePxSize; }
    getModuleY(row) { return this.offsetY + row * this.modulePxSize; }


    drawQrModule(col, row, isBlack) {
    ...
    setupBaseQrGrid() {
        const N = this.qrSize;
        let qrMatrix = Array.from({ length: N }, () => Array(N).fill(2)); 
        ...
        const placeFinderWithSeparators = (colStart, rowStart) => {
            for (let r = 0; r < 7; r++) {
                for (let c = 0; c < 7; c++) {
    ...
    getFormatInfoString() { return "111011110101110".split('').map(b => +b); }
    
    insertFormatInformation(grid, formatBitValues) {
        const N = this.qrSize;
        for (let k = 0; k < 15; k++) {
           
            const pos1 = this.formatBitsPositions[k];
            grid[Math.floor(pos1 / N)][pos1 % N] = formatBitValues[k];

           
            const pos2 = this.formatBitsPositions[k + 15];
            grid[Math.floor(pos2 / N)][pos2 % N] = formatBitValues[k];
        }
    }

    generateErrorCorrection(dataBytes, eccCount) {
        const generatorPoly = FiniteFieldMath.generateRsPoly(this.fieldMath, eccCount);
        let messagePoly = [...dataBytes];
    ...
    applyMaskPattern(qrMatrix) {
        const N = this.qrSize;
        for (let r = 0; r < N; r++) {
            for (let c = 0; c < N; c++) {
                const currentLinearIdx = r * N + c;
    ...
    generateQRCode(inputData) {
        ...
        const N = this.qrSize;
        let qrGridResult = this.setupBaseQrGrid(); 
        ...
        const totalDataBitsCapacity = Object.keys(this.dataModuleIndices).length * 8;

        if (bitPayload.length > totalDataBitsCapacity) {
            console.error("Error: Data exceeds QR Code Version 2-L capacity.");
            for (let y = 0; y < N; y++) for (let x = 0; x < N; x++) this.drawQrModule(x, y, (x+y)%2===0);
            return;
        }
        ...
        const padByte1 = [1,1,1,0,1,1,0,0]; 
        const padByte2 = [0,0,0,1,0,0,0,1]; 
        let currentPad = 0;
        ...
        const eccCodewordsCount = Object.keys(this.eccModuleIndices).length;
        const generatedEccBytes = this.generateErrorCorrection(processedDataCodewords, eccCodewordsCount);
        ...
        for (const blockKey in this.dataModuleIndices) {
            const blockPositions = this.dataModuleIndices[blockKey];
        ...
        for (const blockKey in this.eccModuleIndices) {
            const blockPositions = this.eccModuleIndices[blockKey];
            for (const linearIndex of blockPositions) {
        ...
        const formatBits = this.getFormatInfoString();
    ...
    paint() {
        this.clearCanvasArea();
        const initialGrid = this.setupBaseQrGrid();
        const formatInfo = this.getFormatInfoString();
